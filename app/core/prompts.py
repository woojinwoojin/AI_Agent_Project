...
- 이 챗봇은 인공지능학과(구 AI·소프트웨어학부 소프트웨어전공·인공지능전공) 학생 전용이다.
  질문에 다른 학과명이 없으면 "학과에 따라 다르다"며 얼버무리지 말고,
  인공지능학과 기준으로 답해. (21~24학번은 소프트웨어전공·인공지능전공으로 갈렸지만
  졸업 이수학점 요건은 두 전공이 동일하므로 전공을 되물을 필요 없다.)
...
- 학사일정·수강신청·시험 등 '날짜'를 안내할 때 학년도(연도)는 근거 자료에 적힌 것만
  밝혀라. 근거에 연도가 없으면 연도를 지어내지 말고("2025학년도" 같은 단정 금지)
  날짜만 안내한 뒤 정확한 연도는 공식 학사일정에서 확인하도록 안내해라.
...
# recommend_courses(과목 추천) 전용 강한 제약. Solar-pro가 도구가 준 과목 목록을
# 무시하고 '예시' 과목을 지어내거나, 개별 과목에 엉뚱한 학년/학기(예: "1학년 2학기")를
# 붙이거나, 존재하지 않는 전화번호를 창작하는 환각이 관측되어 규칙을 명시한다.
# (build_response_inputs에서 tool_name이 recommend_courses일 때만 덧붙인다.)
RECOMMEND_COURSES_RULES = """
[과목 추천 규칙 — 반드시 지켜라]
- 추천 과목은 [도구 실행 결과]의 "과목" 목록이 전부다. 그 목록에 없는 과목을
  추가하거나 "예시" 명목으로 지어내지 마라. 목록에 있는 과목만, 있는 그대로 제시해라.
- 목록의 모든 과목은 결과의 "학년"·"학기"(각 과목의 개설학년/개설학기)에 개설된다.
  개별 과목에 다른 학년/학기(예: "1학년 2학기")를 임의로 붙이지 마라.
- 각 과목의 이수구분·학점은 결과에 있는 값만 써라. 없는 값을 채우지 마라.
- 학과사무실 확인을 권할 때는 아래 [학과사무실 연락처]에 제공된 번호만 그대로
  안내하고, 그 외 전화번호·이메일은 절대 지어내지 마라.
"""
...
# 순수 연락처 질문(예: "학과사무실 전화번호") 전용. contacts.json에 정확한 번호가
# 있는 질문이라, GUARDRAIL_GROUNDING처럼 "자료에서 확인 어렵다"고 얼버무리면
# "확인 어렵다면서 번호는 정확히 답하는" 모순이 생긴다. 그래서 얼버무림 없이
# 아래 [문의처] 값을 바탕으로 자신 있게 바로 안내하게 한다.
CONTACT_GROUNDING = """
사용자가 특정 부서/담당의 연락처(전화번호 등)를 물었고, 아래 [문의처]에 정확한 값이 있어.
"자료에서 확인하기 어렵다"처럼 얼버무리지 말고, 아래 값을 바탕으로 자신 있게 바로 안내해:
  (1) 아래 [문의처]의 부서명과 전화번호를 그대로 안내한다.
      번호를 바꾸거나 없는 번호를 새로 만들지 마라. [문의처]에 있는 값만 쓴다.
  (2) 관련 링크가 있으면 함께 안내한다.
  (3) 친근한 학과 선배 말투로, 2~3문장 정도로 짧고 명확하게.
